 ...
 */
app.post('/analyze', (req: Request<object, AnalyzeResponse, AnalyzeRequest>, res: Response) => {
  ...
  switch (mode) {
    case 'timeout':
      ...
      console.log('[mock-analytics] Simulating timeout — no response will be sent');
      setTimeout(() => {
    ...
    case 'partial':
      console.log('[mock-analytics] Simulating partial response (no sentiment field)');
      res.status(200).json({
...
function analyzeSentiment(text: string): 'positive' | 'negative' | 'neutral' {
  ...
  const positiveWords = ['good', 'great', 'excellent', 'happy', 'positive', 'love', 'best'];
  const negativeWords = ['bad', 'terrible', 'awful', 'hate', 'negative', 'worst', 'poor'];
