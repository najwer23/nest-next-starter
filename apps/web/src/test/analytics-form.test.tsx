...
describe('AnalyticsForm', () => {
  ...
  beforeEach(() => {
    ...
    (useAnalyticsStore as unknown as ReturnType<typeof vi.fn>).mockImplementation((selector) =>
      selector({
        refreshHistory,
      }),
    );
  ...
  it('shows analysis in progress state', async () => {
    (analyzeApi as unknown as ReturnType<typeof vi.fn>).mockImplementation(() => new Promise(() => {}));

    render(<AnalyticsForm accessToken="token" />);

    fireEvent.change(screen.getByPlaceholderText('Enter text to analyze...'), {
      target: {
        value: 'hello',
      },
    });
  ...
  it('shows result after successful analysis', async () => {
    (analyzeApi as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({
      sentiment: 'positive',
      keywords: ['nestjs'],
      text: 'hello',
    });
  ...
  it('shows failure and retry button', async () => {
    (analyzeApi as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('Analytics failed'));
