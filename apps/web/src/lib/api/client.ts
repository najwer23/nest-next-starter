...
async function clientFetch<T>(path: string, options: RequestInit = {}, accessToken?: string): Promise<T> {
...
export async function getUsersApi(accessToken: string, page = 1, limit = 20): Promise<PaginatedUsersDto> {
  return clientFetch<PaginatedUsersDto>(`/api/users?page=${page}&limit=${limit}`, {}, accessToken);
}

export async function deactivateUserApi(accessToken: string, id: string): Promise<UserDto> {
  return clientFetch<UserDto>(`/api/users/${id}`, { method: 'DELETE' }, accessToken);
}
